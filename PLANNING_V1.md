...
Découpage en 9 étapes (3-bis et 3-ter sont des étapes intermédiaires insérées au fil du sprint sans renumérotation des étapes structurantes 4-7). Pas de calendrier figé — on avance au rythme des sessions.
...
- [ ] **Étape 3 — Backfill API Popina (caisse seule)** : import depuis `getAllReports` vers `ventes_par_source.popina` + `paiements_caisse` sur la période 16/01/2025 → aujourd'hui. Patch des 16 jours fantômes API (`totalSales=0`) depuis KS2. Idempotence `ON CONFLICT (parametre_id, date, source_id) DO UPDATE`. Uber Eats et la classification fine des paiements `autre` sont traités en 3-bis et 3-ter (cf. session 04/05/2026).
- [ ] **Étape 3-bis — Backfill Uber Eats** : import vers `ventes_par_source.uber_eats` sur la période 16/01/2025 → aujourd'hui depuis KS2.col_I (`uber`) sur 470 jours, avec fallback `entrees.source='uber_eats'` sur les 2 jours sans saisie KS2 (01-02/05/2026). `nb_commandes` Uber via KS2.col_W (`Nb tickets Uber`) à partir du 01/06/2025, `NULL` avant — cf. IRRITANTS_UX_V1.md §F4. Idempotence identique à étape 3.
- [ ] **Étape 3-ter — Classifier paiements paramétrable (F10)** : créer un mécanisme paramétrable par tenant pour classifier les `paymentName` Popina vers les modes de paiement opiniâtres (`especes`/`cb`/`tr`). Pour Krousty : `Foxorder → cb`, `Payxpert → cb`, `Avoir → ignoré` (cf. diagnostic `scripts/diag-paiements-autre.mjs` 04/05/2026 : 421 776 € `autre` = 97% Foxorder + 3% Payxpert sur 16/01/2025 → 02/05/2026). Refacto du classifier hardcodé actuellement dupliqué dans `app/api/cron/nightly/route.js:67-74` et `scripts/import-popina-phase-a-etape-3.mjs:58-65`. Backfill rétroactif des rows `paiements_caisse` déjà écrites en étape 3. **BLOQUANTE avant l'étape 5** : la migration des lectures suppose `paiements_caisse` complet, sinon 35% du CA Krousty manquant. Respecte CLAUDE.md §4 L83 (pas de hardcoding nom plateforme/identité client). Cf. IRRITANTS_UX_V1.md §F10.
...
- **v1.4 (4 mai 2026)** : Sprint Migration data layer — découpage 7 → 9 étapes. Étape 3-bis Uber Eats (séparation source caisse Popina vs source Uber KS2/entrees, suite revirement Q1 session 04/05/2026). Étape 3-ter classifier paiements paramétrable (F10) suite diagnostic Krousty `autre` = 35% du CA (97% Foxorder + 3% Payxpert), BLOQUANTE avant étape 5. Étape 3 portée réduite à popina-caisse seule. F10 hissé V2 → V1 dans `IRRITANTS_UX_V1.md`.

...
export default function DashboardClient({ today, params }) {
  ...
  setData({
    ...
    canaux: { caisse: d.ca.caisse, online: d.ca.online },
  ...
  const caCaisse = data?.canaux?.caisse || data?.ca?.caisse || 0
  const caOnline = data?.canaux?.online || data?.ca?.online || 0
  ...
  const paiements = filtre === 'jour' ? (today?.paiements || {}) : (data?.paiements || today?.paiements || {})
  const commissions = filtre === 'jour' ? (today?.commissions || {}) : (data?.commissions || today?.commissions || {})
  ...
  const seuilJournalier = Math.round(chargesFixesMensuelles / 30 / (tauxMargeVariable / 100))
  const caJour = data?.ca?.brut || today?.ca?.brut || 0
  const seuilAtteint = caJour >= seuilJournalier
  ...
  <div className="space-y-2">
    {[
      { label: 'Borne', val: data?.paiements?.borne || 0, color: 'bg-blue-500' },
      { label: 'Carte bancaire', val: data?.paiements?.cb || 0, color: 'bg-indigo-500' },
      { label: 'Especes', val: data?.paiements?.especes || 0, color: 'bg-yellow-500' },
      { label: 'Titres-restaurant', val: data?.paiements?.tr || 0, color: 'bg-purple-500' },
  ...
  <div className="flex gap-0.5 mt-3 h-1.5 rounded-full overflow-hidden">
    <div className="bg-blue-500" style={{ flex: data?.paiements?.borne || 1 }}></div>
    <div className="bg-indigo-500" style={{ flex: data?.paiements?.cb || 0.01 }}></div>
    <div className="bg-yellow-500" style={{ flex: data?.paiements?.especes || 0.01 }}></div>
    <div className="bg-purple-500" style={{ flex: data?.paiements?.tr || 0.01 }}></div>
  </div>
  <div className="flex justify-between items-center mt-3 pt-3 border-t border-gray-800">
    <span className="text-xs text-gray-400">Commissions estimees</span>
    <span className="text-xs font-mono text-red-400">-{fmt((data?.commissions?.cb || 0) + (data?.commissions?.tr || 0))}</span>
  </div>
</div>
          <div className="bg-gray-900 rounded-2xl p-4 mb-3 border border-gray-800">
  ...
  <div>
    ...
    <p className="text-2xl font-bold font-mono">{fmt(data?.cashADeposer || 0)}</p>
    <p className="text-yellow-700 text-xs mt-1">Especes encaissees sur la periode</p>
  </div>
</div>
          ...
          <div className="w-full rounded-t"
            style={{
              ...
              backgroundColor: isToday
                ? (atteint ? '#22c55e' : '#3b82f6')
                : (atteint ? '#166534' : '#374151')
            }}>
          </div>
        </div>
      )

// Backup JSON local pré-déploiement Phase A étape 4 (dual-write côté cron).
// LECTURE PURE — aucune écriture nulle part.
//
// Tables à dumper :
//   - historique_ca, entrees (legacy/saisie non touchés mais backupés par défense en profondeur)
//   - sources (devrait avoir 2 rows seed Krousty)
//   - ventes_par_source (devrait avoir 1 488 rows = 744 popina + 744 uber_eats post-3-bis)
//   - paiements_caisse (devrait avoir 744 rows post-3-ter)
//
// Sortie :
//   /Users/mRharasse/Downloads/strat-backup-pre-phase-a-etape-4-2026-05-04.json
...
const OUT_PATH = '/Users/mRharasse/Downloads/strat-backup-pre-phase-a-etape-4-2026-05-04.json'
...
const dump = {
  metadata: {
    ...
    snapshot_phase: 'pre-phase-a-etape-4',
    note: 'Backup avant déploiement étape 4 (dual-write côté cron) : refacto cron pour écrire HCA + VPS.popina + PC en parallèle',
  },
  tables: {},
}

console.log('Dump pré-déploiement Phase A étape 4')

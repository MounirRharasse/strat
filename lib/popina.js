...
async function getAllReports(since, until) {
  const reports = []
  let page = 0, totalPages = 1
  while (page < totalPages) {
    const d = await apiGet(`/v1/reports/?since=${since}&until=${until}&index=${page}&size=100`)
    reports.push(...(d.data || []))
    totalPages = d.meta?.totalPage || 1
    page++
  }
  return reports
}
...
async function getAllOrders(since, until) {
  ...
  let page = 0, totalPages = 1
  while (page < totalPages) {
    const d = await apiGet(`/v1/orders/?since=${since}&until=${until}&index=${page}&size=100`)
    orders.push(...(d.data || []))
    totalPages = d.meta?.totalPage || 1
    page++
  }

...
async function _buildCAParJour(parametre_id, periode) {
  ...
  const caParDate = {}
  for (const r of hist) {
    caParDate[r.date] = (caParDate[r.date] || 0) + (r.ca_brut || 0) + (r.uber || 0)
  }
  for (const e of entrUber) {
    caParDate[e.date] = (caParDate[e.date] || 0) + (e.montant_ttc || 0)
  }
...
async function _buildAnomaliesSemaine(parametre_id, periode, parametres) {
  ...
  const [hist, trans, entrees, transHist, ignores] = await Promise.all([
    supabase.from('historique_ca').select('*').eq('parametre_id', parametre_id)
...
export async function buildBriefInputs({ parametre_id, semaine_iso }) {
  ...
  const [kpis4, caParJour, topFourn, anomalies] = await Promise.all([

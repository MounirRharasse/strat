...
async function getCAJour(parametre_id, dateISO) {
  const [hist, entrUber] = await Promise.all([
    supabase.from('historique_ca')
      .select('ca_brut, uber')
      .eq('parametre_id', parametre_id)
      .eq('date', dateISO)
      .then(r => unwrapData(r, 'historique_ca')),
  ...
  let ca = 0
  let hasData = false
  for (const r of hist) { ca += (r.ca_brut || 0) + (r.uber || 0); hasData = true }
  for (const e of entrUber) { ca += (e.montant_ttc || 0); hasData = true }
  return hasData ? ca : null
}
...
export async function evaluerDropOuSpikeCA({ parametre_id, date_ref, parametres }) {
  ...
  const caJour = await getCAJour(parametre_id, dateEval)
  if (caJour == null) return null

  const sources = []
  for (let i = 1; i <= 4; i++) {
    const d = format(subDays(dateEvalObj, 7 * i), 'yyyy-MM-dd')
    const ca = await getCAJour(parametre_id, d)
...
export async function evaluerTransitionSeuil({ parametre_id, date_ref, parametres }) {
  ...
  const [trans30j, hist30j, histMois, entrees30j] = await Promise.all([
    ...
    supabase.from('historique_ca').select('date, ca_ht, uber')
      ...
      .gte('date', debut30j).lte('date', finRef)
      .then(r => unwrapData(r, 'historique_ca')),
    supabase.from('historique_ca').select('date, ca_brut')
      .eq('parametre_id', parametre_id)
      .gte('date', format(debutMois, 'yyyy-MM-dd')).lte('date', finRef)
      .then(r => unwrapData(r, 'historique_ca')),
  ...
  const caHT30j = hist30j.reduce((s, r) => s + (r.ca_ht || 0) + (r.uber || 0) / TVA_UBER_EATS, 0)
    + entrees30j.reduce((s, e) => s + (e.montant_ttc || 0) / TVA_UBER_EATS, 0)

...
import { getDerniereDateAvecCreatedAt, getCaHtSomme } from '@/lib/data/ventes'
import {
  calculerSeuil,
  filtrer30j,
...
async function getSynchroAgeHours(parametre_id) {
  const data = await getDerniereDateAvecCreatedAt(parametre_id)
...
export async function getSeuilRentabilite({ parametre_id }) {
  ...
  const [transRes, caHT] = await Promise.all([
    supabase.from('transactions').select('*')
      .eq('parametre_id', parametre_id).gte('date', debut30j).lte('date', today),
    getCaHtSomme(parametre_id, debut30j, today)
  ])

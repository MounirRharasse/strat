...
export async function getAnomaliesJournal({ parametre_id, since, until }) {
  ...
  // entrees : conservé tel quel — auditerJournal audite la saisie FAB legacy
  // (cf. P1 dette V1+ : entrees.select('*') sans filtre source restera lu directement
  // jusqu'à refonte audit-saisies en V1+)
  const [hist, trans, entrees, transHist, ignores] = await Promise.all([
    getRowsCompatHCA(parametre_id, since6m, _until),

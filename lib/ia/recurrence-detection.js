...
// ─── Constantes ─────────────────────────────────────────────────────
const NB_OCCURRENCES_MIN = 3
const CV_MAX = 0.30                     // coefficient variation montant max (σ/médiane)
const CV_INTERVALLE_MAX = 0.20          // ratio σ/médiane sur intervalles
const FENETRE_JOURS = 180               // 6 mois roulants

// Plages d'intervalle médian acceptées (en jours)
const PLAGES_INTERVALLE = [
  { min: 25, max: 35, freq: 'mensuel' },
  { min: 85, max: 95, freq: 'trimestriel' },
  { min: 175, max: 190, freq: 'semestriel' },
  { min: 355, max: 380, freq: 'annuel' },
]
...
function intervallesEnJours(datesISO) {
  const sorted = [...datesISO].sort()
  const out = []
  for (let i = 1; i < sorted.length; i++) {
    const d1 = new Date(sorted[i - 1] + 'T00:00:00Z')
    const d2 = new Date(sorted[i] + 'T00:00:00Z')
    out.push(Math.round((d2 - d1) / 86400000))
  }
  return out
}

function trouverPlage(intervalleMedian) {
  return PLAGES_INTERVALLE.find(p => intervalleMedian >= p.min && intervalleMedian <= p.max) || null
}
...
export function detecterCandidats(transactions, ignoresFournisseursNorm = new Set()) {
  ...
  // 2. Qualifier chaque groupe
  const candidats = []
  for (const [norm, g] of Object.entries(groupes)) {
    if (g.items.length < NB_OCCURRENCES_MIN) continue

    const montants = g.items.map(i => i.montant)
    const medianMontant = mediane(montants)
    if (medianMontant <= 0) continue
    const sigmaMontant = ecartType(montants)
    const cv = sigmaMontant / medianMontant
    if (cv >= CV_MAX) continue

    const dates = g.items.map(i => i.date)
    const intervalles = intervallesEnJours(dates)
    if (intervalles.length === 0) continue
    const medianIntervalle = mediane(intervalles)
    const sigmaIntervalle = ecartType(intervalles)
    const cvIntervalle = medianIntervalle > 0 ? sigmaIntervalle / medianIntervalle : 1
    if (cvIntervalle >= CV_INTERVALLE_MAX) continue

    const plage = trouverPlage(medianIntervalle)
    if (!plage) continue  // hors plages régulières connues

    // 3. Confiance pct
    const confiance = Math.max(30, Math.min(95, Math.round(
      100 * (1 - cv) * (1 - cvIntervalle) * Math.min(g.items.length / 6, 1)
    )))
    ...
    candidats.push({
      ...
      nb_observations: g.items.length,
      montant_median: Math.round(medianMontant * 100) / 100,
      montant_ecart_pct: Math.round(cv * 100 * 100) / 100,  // en %, 2 décimales
      intervalle_jours_median: Math.round(medianIntervalle),
      derniere_date: sorted[sorted.length - 1],
      premiere_date: sorted[0],

...
export function detecterTrousCanal({
  ...
  // Index uber par date : somme historique_ca.uber + entrees(source=uber_eats).montant_ttc.
  // Les deux sources coexistent : historique_ca.uber pour l'import auto pré-mi-avril 2026,
  // entrees pour la saisie FAB post-mi-avril.
  const uberParDate = {}
  for (const h of (historique || [])) {
    if (!h.date) continue
    uberParDate[h.date] = (uberParDate[h.date] || 0) + (h.uber || 0)
  }
  for (const e of (entrees || [])) {
    if (!e.date || e.source !== 'uber_eats') continue
    uberParDate[e.date] = (uberParDate[e.date] || 0) + (e.montant_ttc || 0)
  }

  // Pré-calcul : pour chaque jour de semaine, les uber > 0 triés par date desc
  const ubersByDow = [[], [], [], [], [], [], []]
  for (const date of Object.keys(uberParDate)) {
    const uber = uberParDate[date]
    if (uber <= 0) continue
    ubersByDow[parseISO(date).getDay()].push({ date, uber })
  }
...
//
// TODO V1.1 — Limite connue : la médiane simple échoue sur les fournisseurs
// à distribution bimodale (ex: Appart City avec loyer mensuel ~1200€ +
// frais ponctuels ~50-100€). Tous les montants ressortent comme "anormaux"
// car la médiane = mid-point entre les 2 clusters.
// Stratégies possibles : clustering K-means, détection bimodalité,
// pondération par catégorie. Voir IRRITANTS_UX_V1.md item 27.
...
export function auditerJournal({
  ...
  const trousCanal = detecterTrousCanal({
    since, today, historique, entrees, ignores
  })

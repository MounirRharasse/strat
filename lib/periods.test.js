...
describe('getPeriodeFromFiltreId', () => {
  it('dispatche vers getCetteSemaine pour "cette-semaine"', () => {
    const refDate = new Date('2026-04-30T10:00:00Z')
    const r = getPeriodeFromFiltreId('cette-semaine', { timezone: TZ, refDate })
    expect(r.filtreId).toBe('cette-semaine')
    expect(r.since).toBe('2026-04-27')
    expect(r.until).toBe('2026-04-30')
  })

  it('dispatche vers getCeMois pour "ce-mois"', () => {
    const refDate = new Date('2026-04-27T10:00:00Z')
    const r = getPeriodeFromFiltreId('ce-mois', { timezone: TZ, refDate })
    expect(r.filtreId).toBe('ce-mois')
    expect(r.since).toBe('2026-04-01')
  })

  it('throw si filtreId inconnu', () => {
    expect(() => getPeriodeFromFiltreId('inexistant', { timezone: TZ })).toThrow(/filtreId inconnu/)
  })

  it('throw si filtreId est "personnalise" (signature différente)', () => {
    expect(() => getPeriodeFromFiltreId('personnalise', { timezone: TZ })).toThrow(/filtreId inconnu/)
  })
})

...
function rebuildMessages(history) {
  ...
  for (const row of history) {
    // Skip rows pré-fix (tool_use_id NULL) pour éviter le 400 Anthropic
    // « unexpected tool_use_id » sur les conversations antérieures à la
    // migration 20260503210000. La conversation perd ses anciens échanges
    // d'outils mais peut continuer sans crash.
    if ((row.role === 'tool_use' || row.role === 'tool_result') && !row.tool_use_id) {
      continue
    }
    if (row.role === 'user') {
      flushAssistant()
      ...
      pendingAssistantBlocks.push({
        type: 'tool_use',
        id: row.tool_use_id, // ID Anthropic, partagé avec le tool_result correspondant
        name: row.tool_name,
      ...
      pendingToolResults.push({
        type: 'tool_result',
        tool_use_id: row.tool_use_id, // appairage explicite (cf. migration 20260503210000)
    ...
    for (const block of toolUseBlocks) {
      ...
      try {
        await saveTurn({
          ...
          tool_use_id: block.id, // ID Anthropic (toolu_XXX), partagé avec tool_result
        })
        await saveTurn({
          parametre_id, conversation_id,
          role: 'tool_result',
          ...
          tool_use_id: block.id, // même valeur — c'est l'appairage côté rebuildMessages
        })
      } catch (e) {

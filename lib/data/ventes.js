...
export const SOURCE_UBER_EATS_ID = '888b047c-54d9-4c05-a364-5e1a9c6a9409'
 ...
 *
 * @returns Array<{ date, ca_brut, ca_ht, especes, cb, tpa, tr, uber, nb_commandes, nb_commandes_uber, commission_uber }>
 *          ca_brut = popina TTC + uber TTC (sémantique TOTAL, cohérent décision Q3 étape 5).
 *          ca_ht   = popina HT + uber TTC / TVA_UBER_EATS (sémantique TOTAL HT, cohérent ca_brut).
 *          uber    = montant_ttc VPS uber_eats SÉPARÉMENT (legacy historique_ca.uber).
 *          tpa     = 0 (fusion D3 appliquée à l'écriture, cf. étape 3-ter).
 *          uber    = 0 si pas de ligne VPS uber_eats (legacy : 0 si pas de saisie).
 *          nb_commandes      = VPS popina.nb_commandes (caisse Restaurant, post-Lot 3).
 *          nb_commandes_uber = VPS uber_eats.nb_commandes (= "nb commandes Uber" du
 *          legacy historique_ca.nb_commandes par convention pré-migration).
 *          commission_uber = null (colonne legacy historique_ca jamais peuplée
 *          par le cron, fidèle au comportement pré-migration).
 *          Inclut toutes les dates où popina OU uber existe dans la période.
 *
 * Note pour les consommateurs : si vous faites `caHistBrut + sum(uber)` en
 * cumulant, vous double-comptez l'uber. ca_brut et ca_ht sont déjà TOTAL.
 *
 * ⚠ Sémantique nb_commandes : le legacy historique_ca.nb_commandes était par
 * convention "nb commandes Uber Eats" (cf. analyses-kpis.js L108-109 commit pré-Lot 4).
 * Désormais nb_commandes = caisse Restaurant, nb_commandes_uber = Uber Eats.
 * Migrer les call-sites legacy qui consommaient nb_commandes comme "uber" vers
 * nb_commandes_uber.
 */
export async function getRowsCompatHCA(parametreId, dateMin, dateMax) {
  ...
  return [...allDates].sort().map(date => {
    ...
    const popinaTtc = p ? Number(p.montant_ttc || 0) : 0
    const popinaHt  = p ? Number(p.montant_ht || 0)  : 0
    const uberTtc   = u ? Number(u.montant_ttc || 0) : 0
    return {
      date,
      ca_brut: r2(popinaTtc + uberTtc),  // TOTAL : popina TTC + uber TTC (cohérent Q3)
      ca_ht: r2(popinaHt + uberTtc / TVA_UBER_EATS),  // TOTAL HT : popina HT + uber HT (TVA 10%)
      ...
      nb_commandes_uber: u ? u.nb_commandes : null,  // sémantique "nb commandes Uber" legacy
      commission_uber: null,  // F6 — colonne legacy jamais peuplée, fidélité legacy stricte
    }
  })
}
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  console.log(`  getCaBrut : ${ttc1} (popina + uber, post-comblement Lot 3)`)
  if (ttc1 < 6200 || ttc1 > 6300) throw new Error(`getCaBrut hors plage attendue [6200, 6300], got ${ttc1}`)
  const platfm1 = await getCaPlateformes(k, '2026-05-03')
  console.log(`  getCaPlateformes : ${platfm1} (uber 03/05 backfillé Phase 0 Lot 3)`)
  if (platfm1 === null || platfm1 < 3500 || platfm1 > 3800) throw new Error(`getCaPlateformes hors plage attendue [3500, 3800], got ${platfm1}`)
  ...
  if (r0.commission_uber !== null) throw new Error(`getRowsCompatHCA commission_uber attendu = null (F6 legacy), got ${r0.commission_uber}`)

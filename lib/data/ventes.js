 ...
 *
 * @returns Array<{ date, ca_brut, ca_ht, especes, cb, tpa, tr, uber, nb_commandes, commission_uber }>
 *          ca_brut = popina TTC SEUL (sémantique legacy historique_ca.ca_brut, n'inclut PAS l'uber).
 ...
 *          par le cron, dette F6 IRRITANTS — fidèle au comportement pré-migration).
...
export async function getRowsCompatHCA(parametreId, dateMin, dateMax) {
  ...
  return [...allDates].sort().map(date => {
    ...
    return {
      date,
      ca_brut: p ? r2(Number(p.montant_ttc || 0)) : 0,  // popina SEUL (sémantique legacy HCA.ca_brut)

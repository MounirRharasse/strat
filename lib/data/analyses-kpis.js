...
export async function getAnalysesKPIs({ parametre_id, since, until, parametres }) {
  ...
  if (needsPopina) {
    ...
    caBrut = caHistBrut + caUberTotal
    caHT = caHistHT + (caUberTotal / TVA_UBER_EATS)
    tva = caBrut - caHT
  ...
  if (stockDebut && stockFin && !needsPopina) {
    ...
    const uberHistExact = histExact.reduce((s, r) => s + (r.uber || 0), 0)
    const uberManuelExact = entreesUberPourExact
      .filter(e => e.date >= debutDate && e.date <= finDate)
      .reduce((s, e) => s + (e.montant_ttc || 0), 0)
    const caHTExact = caHistExact + (uberHistExact + uberManuelExact) / TVA_UBER_EATS

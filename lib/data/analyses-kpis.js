  ...
  if (stockDebut && stockFin && !needsPopina) {
    ...
    if (debutDate < since) {
      const [{ data: histAvant }, { data: entreesAvant }, { data: transactionsAvant }] = await Promise.all([
        supabase.from('historique_ca').select('*').gte('date', debutDate).lt('date', since),
        supabase.from('entrees').select('*').gte('date', debutDate).lt('date', since).eq('source', 'uber_eats'),
        supabase.from('transactions').select('*').gte('date', debutDate).lt('date', since)
      ])

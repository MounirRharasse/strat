...
export async function getAnalysesKPIs({ parametre_id, since, until, parametres }) {
  ...
  // Migration étape 5 Lot 3 : historique + entrees uber → getRowsCompatHCA (1 fetch agrégé via VPS+PC).
  // Note : entreesUber devient [] car les saisies FAB Uber sont mergées dans VPS uber_eats via étape 3-bis
  // (et Phase 0 Lot 3 a comblé jusqu'au 03/05/2026). La structure rows reste compatible legacy historique_ca.
  const [{ data: transactions }, historique, { data: inventaires }] = await Promise.all([
  ...
  const entreesUber = []  // saisies FAB Uber désormais lues via VPS uber_eats dans historique
  ...
  if (needsPopina) {
    ...
    caHT = caHTPopina + (caUberTotal / TVA_UBER_EATS)
    ...
    // Migration étape 5 Lot 4 : nb_commandes_uber = VPS uber_eats.nb_commandes (sémantique
    // "nb commandes Uber" du legacy historique_ca.nb_commandes par convention pré-migration).
    // Fix dette signalée commit Lot 3.
    ...
    // Migration étape 5 Lot 3 : `historique` vient de getRowsCompatHCA, qui retourne
    // ca_brut et ca_ht DÉJÀ EN TOTAL (popina + uber). Pas de double-addition uber.
    const uberHist = historique.reduce((s, r) => s + (r.uber || 0), 0)
    caUberTotal = uberHist  // entreesUber=[] post-Lot 3, uberHist reflète VPS uber_eats
    const caHistBrut = historique.reduce((s, r) => s + (r.ca_brut || 0), 0)
    const caHistHT = historique.reduce((s, r) => s + (r.ca_ht || 0), 0)
    caBrut = caHistBrut  // déjà TOTAL via getRowsCompatHCA
    caHT = caHistHT      // déjà TOTAL via getRowsCompatHCA
    tva = caBrut - caHT
    const caHorUber = caHistBrut - uberHist  // = popina seul (≥ 0 par construction)
    ...
    // Migration étape 5 Lot 4 : nb_commandes_uber = VPS uber_eats.nb_commandes (sémantique
    // "nb commandes Uber" du legacy). entreesUber=[] post-Lot 3.
    // Fidélité legacy stricte : pour période passée, nbCommandes = uber seul (commentaire d'origine
    // ci-dessus préservé en comportement). Le fait que la caisse popina ne soit pas comptée ici est
    // un choix legacy hérité (cf. ancien commentaire "pas de Popina sur période passée").
    const nbCmdUber = historique.reduce((s, r) => s + (r.nb_commandes_uber || 0), 0)
    nbCommandes = nbCmdUber
    nbCommandesUber = nbCmdUber
  }
  ...
  if (stockDebut && stockFin && !needsPopina) {
    ...
    if (debutDate < since) {
      // Migration étape 5 Lot 3 : historique + entrees uber → getRowsCompatHCA filtré côté JS (lt since strict).
      const [histAvantBrut, { data: transactionsAvant }] = await Promise.all([
        getRowsCompatHCA(parametre_id, debutDate, since),
        supabase.from('transactions').select('*').eq('parametre_id', parametre_id).gte('date', debutDate).lt('date', since)
      ])
      const histAvant = histAvantBrut.filter(r => r.date < since)  // [debutDate, since[ strict
    ...
    // Migration étape 5 Lot 4 : histPourExact vient de getRowsCompatHCA, ca_ht TOTAL.
    // entreesUberPourExact = [] post-Lot 3 (saisies FAB merged dans VPS uber_eats).
    // Retiré `+ uberHistExact/TVA + uberManuelExact/TVA` qui doublait l'uber HT.
    const caHTExact = histExact.reduce((s, r) => s + (r.ca_ht || 0), 0)
